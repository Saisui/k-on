...
class Integer
  ...
  def deg
    self.to_s.split("").map{|a|a.to_i}
  end
  def ps op,*as
    [self] + as.map{|a| self.send(op,a)}
  end
end
...
class Array
  ...
  def >> sq
    ...
    when Array
      sq.each do |a|
        res << a-1==-1 ? nil : self[a]
      end
    when String
      res = sq.split("").map{|a| a.to_i-1==-1 ? nil : self[a.to_i-1] } 
    when Integer
      res = sq.to_s.split("").map{|a| a.to_i-1==-1 ? nil : self[a.to_i-1]}
    end
    res
  end
end
